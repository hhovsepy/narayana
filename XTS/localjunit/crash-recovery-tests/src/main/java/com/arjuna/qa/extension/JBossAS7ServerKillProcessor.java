...
public class JBossAS7ServerKillProcessor extends BaseServerKillProcessor {
    ...
    private static final String PROCESSES_CMD = ServerExtension.isIbmJdk() ? (ServerExtension.isSolaris() ? "/usr/ucb/ps aux" : "ps aux") : "jps"; // IBM JDK does not have "jps" so using "ps", Solaris have "ps" in "/usr/ucb/".
    ...
    private static final String SHUTDOWN_JBOSS_CMD = PROCESSES_CMD + " | grep jboss-module[s] | awk '" + (ServerExtension.isIbmJdk() ? "{print $2}" : "{print $1}") + "' | xargs kill";
